...
const authentication = async (req, res, next) => {
    ...
    try{
        ...
        debug('[authentication] isNewUser:', req.session?.user?.isNewUser);
        next();
    } catch(error){
...
const authenticationRole = (role) => {
    return (req, res, next) => {
        ...
        try{
            ...
            console.log('[authenticationRole] Error:',error)
            throw new UnauthenticatedError('Invalid authentication credentials provided.');
        }
    }
}
...
const checkSession = (req, res, next) => {
    if (req.session?.user) {
        debug(`[checkSession] isNewUser: ${req.session?.user?.isNewUser}`);
        return next();
    }
...
const checkSessionRole = (role) => {
    return (req, res, next) => {
        if (req.session?.user && req.session?.user?.role === role) {
            debug(`[checkSessionRole] Session role: ${req.session?.user?.role}`);
            return next();  
        }
        debug(`[checkSessionRole] Session role: ${req.session?.user?.role}`);
        return res.redirect('/login');
...
const logout = async (req, res, next) => {
    try {
        ...
        const cookies = ['connect.sid', 'jwt', 'hasToken', 'lang'];
